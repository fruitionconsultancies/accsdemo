...
function renderProduct(product, config, block) {
  const {
    name, urlKey, sku, price, priceRange, addToCartAllowed, __typename, description,
  } = product;
  ...
  const fragment = document.createRange().createContextualFragment(`
    ...
    <div class="details">
      ...
      <div class="actions">
        ...
        ${config['cart-button'] && addToCartAllowed && __typename === 'SimpleProductView' ? '<button class="add-to-cart button secondary">Add to Cart</button>' : ''}
      </div>
    </div>
  `);
